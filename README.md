...
* `Output.txt` - результат перемножения матриц из образца, используя MPI (4 процессора).
...
Пример запуска: mpiexec -n 1 ./main.exe
    где -n колмичество процессоров
```

После запуска программы в консоль будет выведена информация о время перемножения матриц всех размеров с указанием:
* количества использованных процессоров
* занятого времени

Пример результата - `results.txt`

Так же будут сохранены исходные матрицы 1200x1200 и результат их умножения используя MPI (4 процессора) для проверки.
...
### Реализация параллельной работы по технологии MPI (внутри Matrix.h)
...
```
Пример запуска: python verify.py -a InputA.txt -b nputB.txt -o Output.txt
Пример запуска: python verify.py
```

В данной лабораторной работе результат проверил только на одной из матриц, а имеено при перемножении матриц 1200x1200 с использованием MPI (4 процессоров) - результат сошёлся. Посчитал этого достаточным доказательством того, что распараллеливание работает правильно.

---

### Выводы
* Была реализована программа параллельного перемножения квадратных матриц на языке C++ с использованием MPI.
* Лучшее ускорение: 4.062 при 8 процессорах на матрицах 200x200. Нелинейный рост ускорения после 4 потоков объясняется архитектурой процессора - всего 4 ядра.
* Лучшая эффективность: 1.003 при 2 процессах на матрицах 2000x2000.
* При матрицах небольших размеров, использование большого количества процессоров не имеет смысла, из-за того что время на копировании данных для самих процессоров соизмеримо с временем вычисления матрицы. (Видно на матрице 400x400 на четырех и восьми процессорах).
